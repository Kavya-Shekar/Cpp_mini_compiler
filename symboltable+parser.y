...
ASSIGN
	: T_identifier '=' EXP    {search_id($1,yylineno);update($1,yylineno,$3);}
	;
...
STATEMENTS
	...
	| UX
	| PRINT 
	| T_identifier Function_call {search_id($1,yylineno);}
	|
	;	
  ...
  if(!yyparse())  //yyparse-> 0 if success
  {
    ...
    for(int i = 0;i < struct_index;i++)
    {
      ...
      if(st[i].type=='F')
      {
        ...
        ty= "identifier";
        fprintf(fptr,"%-4d\t  %-7s\t   %-10d\t %-9s\t  NULL\t\t %-5s\t  %-4d\n",i+1,st[i].name,st[i].line,ty,st[i].value,st[i].scope);
    }
    }
    fclose(fptr);
  }
